...
export const MyPostsList = () => {
  const { posts, postsInfo, currentUser, search } = useContext(BlogContext);
  return (
    <ol className="p-5 pt-2 mt-3 flex gap-5 item-start justify-center flex-wrap overflow-hidden overflow-y-auto">
      {posts.map(({ id, userId, title, body }) => {
        ...
        if (userId == currentUser?.login.userId) {
